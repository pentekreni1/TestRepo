...
public class AiGameBoard implements BoardSubject, IGameBoard {
    private static AiGameBoard instance = new AiGameBoard();
    ...
    public static AiGameBoard getInstance() {
        return instance;
    }
    ...
    public void nextPlayer() {
        ...
        if (state.getColor() == Constant.RED) {
            state = (!points[Constant.BLACKBAR].empty()) ? blackBarState : blackState;
            aiGameController.rollDice();
            List<Die> dice = aiGameController.getGame().getDice();

            //todo wait

            while (dice.size() > 0){
                Move move = aiGameController.getGame().getBestMove();
                Checker checker = aiGameController.getChecker(move.getFromPosition());
                Circle circle = aiGameController.getCircle(checker);
                aiGameController.drawMovedCheckersByAi(circle, move);
                int diceValue = Math.abs(move.getFromPosition() - move.getToPosition());
                Die dieToRemove = new Die(diceValue);
    ...
    public void setController(AiGameController aiGameController) {
        this.aiGameController = aiGameController;
    }
}